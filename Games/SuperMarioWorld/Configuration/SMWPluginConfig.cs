...
namespace Retro_ML.SuperMarioWorld.Configuration
{
    ...
    {
        public (string fieldName, Type type)[] Fields => new (string, Type)[]
        {
            (nameof(UseGrid), typeof(bool))
        };
        ...
        public void InitNeuralConfig(NeuralConfig neuralConfig)
        {
            ...
            if (UseGrid)
            {
                neuralConfig.InputNodes.Add(new InputNode("Grid Tiles", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => tilesFunc((SMWDataFetcher)dataFetcher, GridDistanceX, GridDistanceY), GridWidth, GridHeight));
                neuralConfig.InputNodes.Add(new InputNode("Grid Dangers", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => dangersFunc((SMWDataFetcher)dataFetcher, GridDistanceX, GridDistanceY), GridWidth, GridHeight));
                neuralConfig.InputNodes.Add(new InputNode("Grid Goodies", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => goodiesFunc((SMWDataFetcher)dataFetcher, GridDistanceX, GridDistanceY), GridWidth, GridHeight));
                neuralConfig.InputNodes.Add(new InputNode("Grid Water", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => waterFunc((SMWDataFetcher)dataFetcher, GridDistanceX, GridDistanceY), GridWidth, GridHeight));
            }
            else
            {
                neuralConfig.InputNodes.Add(new InputNode("Ray Tiles", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => Raycast.GetRayDistances(tilesFunc((SMWDataFetcher)dataFetcher, RayLength, RayLength), RayLength, RayCount), RaysWidth, RaysHeight));
                neuralConfig.InputNodes.Add(new InputNode("Ray Dangers", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => Raycast.GetRayDistances(dangersFunc((SMWDataFetcher)dataFetcher, RayLength, RayLength), RayLength, RayCount), RaysWidth, RaysHeight));
                neuralConfig.InputNodes.Add(new InputNode("Ray Goodies", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => Raycast.GetRayDistances(goodiesFunc((SMWDataFetcher)dataFetcher, RayLength, RayLength), RayLength, RayCount), RaysWidth, RaysHeight));
                neuralConfig.InputNodes.Add(new InputNode("Ray Water", neuralConfig.EnabledStates[enabledIndex++], (dataFetcher) => Raycast.GetRayDistances(waterFunc((SMWDataFetcher)dataFetcher, RayLength, RayLength), RayLength, RayCount), RaysWidth, RaysHeight));
            }
